...
document.getElementById('formChamado').addEventListener('submit', async (e) => {
    ...
    let slaHoras;
    switch(prioridade) {
        case 'baixa': slaHoras = 72; break;
        case 'media': slaHoras = 48; break;
        case 'alta': slaHoras = 24; break;
        default: slaHoras = 48;
    }
...
function mostrarSucesso(id) {
    ...
    let slaTexto;
    switch(prioridade) {
        case 'baixa': slaTexto = 'Até 72 horas'; break;
        case 'media': slaTexto = 'Até 48 horas'; break;
        case 'alta': slaTexto = 'Até 24 horas'; break;
        default: slaTexto = 'Até 48 horas';
    }
    
    document.getElementById('protocoloNumero').textContent = protocolo;
    document.querySelector('.sla-info p').innerHTML = `🕰️ <strong>Tempo de resposta:</strong> ${slaTexto}`;
...
async function buscarChamado() {
    ...
    try {
        const response = await fetch(`http://host67.expnac.local:5000/chamados/${protocolo}`);
