...
function aplicarFiltros() {
  ...
  const chamadosFiltrados = todosOsChamados.filter((chamado) => {
    ...
    const matchSolicitante =
      !solicitante || normalizarTexto(chamado.solicitante).includes(solicitanteNormalizado)
...
function criarCard(chamado) {
  ...
  // Debug detalhado
  console.log(
    `Criando card #${chamado.id} - Status: ${chamado.status}, Prioridade: ${chamado.prioridade || 'media'}`,
  )
  ...
  card.innerHTML = `
        ...
        ${
          chamado.analista
            ...
            : '<div class="card-sem-responsavel">📋 Não atribuído</div>'
        }
        <div class="card-sla">
    ...
    if (newStatus === 'finalizado') {
      ...
      if (chamado && chamado.equipamento === 'sim') {
        const confirmacao = confirm(
          '💻 O equipamento foi levado ao suporte para configuração?\n\nClique OK se SIM ou Cancelar se NÃO.',
        )
        if (!confirmacao) {
...
async function abrirDetalhes(id) {
  ...
  const prioridadeTexto =
    c.prioridade === 'alta' ? '🔥 Alta' : c.prioridade === 'baixa' ? '🟢 Baixa' : '🟡 Média'
                    ...
                    <div class="analista-section">
                        ...
                        <select id="selectAnalista">
                            ...
                            ${analistas
                              .slice(1)
                              .map(
                                (a) => `<option value="${a}" ${a === c.analista ? 'selected' : ''}>
                                👤 ${a}
                            </option>`,
                              )
                              .join('')}
  ...
  if (selectAnalista) {
    selectAnalista.onchange = async (e) => {
      ...
      try {
        ...
        if (response.ok) {
          // Sucesso
          statusEl.innerHTML = e.target.value
            ? `✅ Atribuído para ${e.target.value}`
...
document.getElementById('btnEditar').onclick = () => {
  if (chamadoAtual) {
    ...
    document.querySelector(`input[name='equip'][value='${chamadoAtual.equipamento}']`).checked =
      true
...
async function cancelarChamado(id) {
  if (
    confirm(
      '⚠️ Tem certeza que deseja cancelar este chamado?\n\nEle será mantido no histórico para relatórios.',
    )
  ) {
    const usuario = sessionStorage.getItem('crmUser')
...
async function atribuirParaMim(id, event) {
  ...
  try {
    const response = await fetch(`http://host67.expnac.local:3003/chamados/${id}/analista`, {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        analista: usuario.charAt(0).toUpperCase() + usuario.slice(1),
        usuario,
      }),
    })
...
function calcularTempoDecorrido(dataCreated) {
  ...
  console.log(
    `Tempo Debug - Data: ${dataCreated}, Criado: ${criado.toLocaleString('pt-BR')}, Agora: ${agora.toLocaleString('pt-BR')}, Diff: ${diff}ms, Minutos: ${minutos}, Horas: ${horas}`,
  )
