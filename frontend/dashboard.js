...
function atualizarMetricasSLA() {
    ...
    const chamadosAtivos = dadosChamados.filter(c => 
    ...
    const chamadosComFinalizacao = dadosChamados.filter(c => 
        c.status === 'finalizado' && c.created_at && c.finished_at
    );
    let tempoMedio = 0;
...
function criarGraficoAnalistas() {
    ...
    const dados = analistas.map(analista => 
        dadosChamados.filter(c => c.analista === analista).length
    );
...
function criarGraficoSetores() {
    ...
    const dados = setores.map(setor => 
        dadosChamados.filter(c => c.setor === setor).length
    );
...
function criarGraficoAcessos() {
    ...
    const dados = acessosUnicos.map(acesso => 
        todosAcessos.filter(a => a === acesso).length
    );
    ...
    graficos.acessos = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                data: dados,
                backgroundColor: [
                    '#f40f2b', '#cd1040', '#a41056', '#821068', '#5e117b'
                ],
                borderWidth: 2,
                borderColor: '#fff'
            }]
...
function atualizarGraficoStatus() {
    ...
    graficos.status.data.datasets[0].data = [stats.aberto, stats.atendimento, stats.finalizado, stats.cancelado];
...
function calcularDadosTendencia() {
    ...
    let totalFinalizados = dadosChamados ? dadosChamados.filter(c => c.status === 'finalizado').length : 0;
                        ...
                        <div class="info-item">
                            ...
                            <div class="acessos-list">
                                ${chamado.flags && chamado.flags.length > 0 ? 
                                    chamado.flags.map(flag => `<span class="acesso-tag">${flag}</span>`).join('') : 
